import {ref, computed, useTemplateRef, watchEffect, onMounted, type Ref, nextTick  } from 'vue'
...
export const useTodoManager = (inputRef?: Ref<HTMLInputElement | null>) => {
  ...
  { id: 2, text: 'Написать композабль', completed: true, createdAt: new Date() }
])
  const newTodoText = ref('')
  const filter = useLocalStorage<'all' | 'active' | 'completed'>('filter', ref<'all' | 'active' | 'completed'>('all'))
...
const addTodo = () => {
  ...
  if (!editedTodoId.value) {
    const newTodo: Todo = {text, completed: false, createdAt: new Date(), id: Date.now()}
